...
/**
 * Emits bytecode for `ReflectAOTRegistry`, the `ReflectAOTRuntime` implementation that `instanceof` dispatches to the
 * per-type accessor classes emitted by [AccessBytecodeEmitter].
 */
object RegistryBytecodeEmitter {

  /** Internal JVM name of the generated registry class. */
  ...
  private val M_FOR_EACH_FIELD = AsmMethod(ReflectApiNames.FOR_EACH_FIELD, Type.VOID_TYPE, arrayOf(OBJECT_TYPE, Type.getType(java.util.function.BiConsumer::class.java)))
  private val M_FOR_EACH_PROPERTY = AsmMethod(ReflectApiNames.FOR_EACH_PROPERTY, Type.VOID_TYPE, arrayOf(OBJECT_TYPE, Type.getType(java.util.function.BiConsumer::class.java)))
  ...
  private val M_FOR_EACH_METHOD = AsmMethod(ReflectApiNames.FOR_EACH_METHOD, Type.VOID_TYPE, arrayOf(CLASS_TYPE, BICONSUMER_TYPE))
  ...
  /**
   * Writes the registry class after accessors exist for every entry in [types].
   *
   * @param types Sorted or unsorted list of introspected types. Implementations sort by internal name for stable bytecode.
   * @param outputDir Root directory that will receive generated bytecode.
   * @param methodBindings Resolved `Reflect.method` bindings (used for `forEachMethod` class dispatch).
   */
  fun emit(types: List<TypeIntrospection.IntrospectedType>, outputDir: File, methodBindings: List<MethodIdBinding>) {
    ...
    emitDispatchObjectBiConsumerVoid(cw, M_FOR_EACH_FIELD, sorted, ReflectApiNames.FOR_EACH_FIELD) { t ->
      val owner = Type.getObjectType(t.internalName)
      AsmMethod(ReflectApiNames.FOR_EACH_FIELD, Type.VOID_TYPE, arrayOf(owner, BICONSUMER_TYPE))
    }

    emitDispatchObjectBiConsumerVoid(cw, M_FOR_EACH_PROPERTY, sorted, ReflectApiNames.FOR_EACH_PROPERTY) { t ->
      val owner = Type.getObjectType(t.internalName)
      AsmMethod(ReflectApiNames.FOR_EACH_PROPERTY, Type.VOID_TYPE, arrayOf(owner, BICONSUMER_TYPE))
    }

    emitDispatchClassBiConsumerVoid(cw, M_FOR_EACH_METHOD, sorted, methodBindings)
  ...
  /**
   * `copy(Object)` dispatches to typed `Foo_ReflectAOT.copy(Foo)`, else [ReflectAOTDefaultDispatch.copy].
   */
  ...
  private fun emitDispatchObjectBiConsumerVoid(
    cw: ClassWriter,
    ifaceMethod: AsmMethod,
    sorted: List<TypeIntrospection.IntrospectedType>,
    reflectMethodNameForErrors: String,
    ...
    if (sorted.isEmpty()) {
      ...
      ga.invokeStatic(
        DEFAULT_TYPE,
        AsmMethod(reflectMethodNameForErrors, Type.VOID_TYPE, arrayOf(OBJECT_TYPE, BICONSUMER_TYPE)),
      )
      ga.returnValue()
      ga.endMethod()
      return
  ...
  /**
   * Dispatches `forEachMethod(Class, BiConsumer)` by reference-comparing the class operand to each specialized type
   * that owns at least one `Reflect.method` binding, then invokes the typed accessor `forEachMethod(BiConsumer)`.
   */
  private fun emitDispatchClassBiConsumerVoid(
    cw: ClassWriter,
    ifaceMethod: AsmMethod,
    sorted: List<TypeIntrospection.IntrospectedType>,
    ...
    if (sorted.isEmpty()) {
      ga.loadArg(0)
      ga.loadArg(1)
      ga.invokeStatic(DEFAULT_TYPE, ifaceMethod)
      ga.returnValue()
      ga.endMethod()
      return
    }
    ...
    for (t in sorted) {
      ...
      val next = ga.newLabel()
      ga.loadArg(0)
      ga.visitLdcInsn(Type.getObjectType(t.internalName))
      ga.visitJumpInsn(Opcodes.IF_ACMPNE, next)
      val access = Type.getObjectType(AccessBytecodeEmitter.accessInternalName(t.internalName))
      ga.loadArg(1)
      ga.invokeStatic(
        access,
        AsmMethod(ReflectApiNames.FOR_EACH_METHOD, Type.VOID_TYPE, arrayOf(BICONSUMER_TYPE)),
      )
      ga.returnValue()
      ga.mark(next)
    }
