...
/**
 * Parses `.class` bytes into field lists, JavaBeans-style properties, and public instance methods used by scanning and emission.
 *
 * Responsibilities include merging superclass metadata so subclass fields shadow correctly, collecting override-stable
 * instance methods for `Reflect.method` validation, and exposing small helpers such as [loadClassBytes] for other
 * codegen utilities.
 */
object TypeIntrospection {
  ...
  /**
   * Maps JVM access flags to a short English visibility label used in user-facing error strings.
   *
   * @param access Raw `Opcodes.ACC_*` bitmask from ASM field or method nodes.
   * @return One of `public`, `protected`, `private`, or `package-private`.
   */
  ...
  /**
   * One public, non-static, non-final instance field that generated bytecode may read or write with `getfield` and `putfield`.
   *
   * @property name JVM field name.
   * @property descriptor Field type descriptor such as `I` or `Ljava/lang/String;`.
  ...
  /**
   * Loads [internalName] from [roots], walks superclasses, and returns a merged [IntrospectedType] snapshot.
   *
   * @param internalName JVM internal name of the leaf class to load.
   * @param roots Classpath entries used to locate class bytes for the leaf and its supers.
   * @return Merged metadata, or null when the leaf class bytes cannot be read from [roots].
   */
  ...
  /**
   * Returns whether the leaf class exposes a public no-argument instance constructor.
   *
   * @param internalName Class internal name.
   * @param roots Classpath containing the `.class` file.
   * @return True when a `()V` `<init>` exists with `ACC_PUBLIC`.
   */
  ...
  /**
   * Builds an ordered superclass chain starting at [start] and stopping before `java/lang/Object`.
   *
   * @param start Leaf [ClassNode] loaded from user bytes.
   * @param roots Classpath entries used to read each superclass layer.
   * @return Ordered list from leaf toward `java/lang/Object`, excluding `java/lang/Object` itself.
   */
  ...
  /**
   * Maps a JavaBeans property name to an exact field name when the property is backed by a public field with the same name.
   *
   * @param prop Property base name inferred from getter or setter names.
   * @param fieldNames Set of field names declared on the merged type model.
   * @return The field name when present, otherwise null.
   */
  ...
  /**
   * Lowercases the first character of [s] for conventional JavaBeans property naming, preserving acronyms when needed.
   *
   * @param s Non-empty identifier fragment such as `URL` or `Name`.
   * @return A decapitalized form suitable for matching field names derived from accessor names.
   */
  ...
  /**
   * Reads raw `.class` bytes for [internalName] from directory trees or JAR files in [roots].
   *
   * @param internalName JVM internal name without the `.class` suffix.
   * @param roots Classpath entries produced by the Gradle task (compiled outputs and dependencies).
